(() => {
  ...
  const multiProveedor = crearMultiSelect("multiselect-proveedor", () => render());
  ...
  function poblarFiltros() {
    const proveedores = [...new Set(articulos.map((a) => a.proveedor))].sort((a, b) => a.localeCompare(b, "es"));
    const rubros = [...new Set(articulos.map((a) => a.rubro))].sort((a, b) => a.localeCompare(b, "es"));
    multiProveedor.setOpciones(proveedores);
  ...
  // Filtros "de contexto" (proveedor/rubro/buscador), sin el filtro de estado -
  ...
  function filtrarSinEstado(lista) {
    const proveedores = multiProveedor.getSeleccion();
    ...
    return lista.filter((a) => {
      ...
      if (!proveedores.has(a.proveedor)) return false;
  ...
  function render() {
    ...
    if (filas.length === 0) {
      cuerpoTabla.innerHTML = '<tr><td colspan="10">No hay artículos que coincidan con los filtros.</td></tr>';
    } else {
      ...
      <tr class="${clase}">
        ...
        <td class="num">${valorCeldaTexto(art, "promedio_diario_bultos")}</td>
  ...
  cargarTodo().catch((err) => {
    ...
    cuerpoTabla.innerHTML = '<tr><td colspan="10">No se pudieron cargar los artículos.</td></tr>';
  });
})();
